...
<div class="row-fluid">
    ...
    <div class="span3">
    	...
    	<h2>Test send this campaign</h2><br/>
	    ...
	    <h2>Define recipients</h2><br/>
	        ...
	        <input type="hidden" name="path" value="{{ STATIC_URL }}">
	        ...
	        <?php if(get_app_info('is_sub_user')):?>
		        ...
		        <?php if(paid()):?>
			        ...
			        <?php if($brand_monthly_quota!=-1):?><strong>Monthly limit</strong>: <?php echo $brand_monthly_quota.' ('._('resets on').' '.$brand_month_of_next_reset.' '.$brand_limit_resets_on;?>)<br/><?php endif;?>
		        	<strong>Recipients</strong>: <span id="recipients">0</span> 
		        ...
		        <?php else:?>
			        ...
			        <div class="well" style="width:260px;">
				        ...
				        <strong>Delivery Fee</strong>: <?php echo get_fee('currency');?> <span id="delivery_fee"><?php echo get_fee('delivery_fee');?></span><br/>
				        <strong>Fee per recipient</strong>: <?php echo get_fee('currency');?> <span id="recipient_fee"><?php echo get_fee('cost_per_recipient');?></span><br/><br/>
				        <span class="grand_total"><strong>Grand total</strong>: <?php echo get_fee('currency');?> <span id="grand_total">0</span></span>
			        </div>
			        ...
			        <button type="submit" class="btn btn-inverse btn-large" id="pay-btn" <?php echo $send_newsletter_now;?>><i class="icon-arrow-right icon-white"></i> Proceed to pay for campaign</button>
			        <p style="margin-top:10px; text-decoration:underline;">
			        	<?php if($cron):?>
			        	<a href="javascript:void(0)" id="send-later-btn"><?php echo $send_newsletter_text;?></a>
			        	<?php endif;?>
		    ...
		    <?php else:?>
		    	...
		    	<strong>Recipients</strong>: <span id="recipients">0</span> <?php echo $aws_keys_available=='true' ? _('of') : '';?> <?php echo $aws_keys_available=='true' ? $ses_sends_left : ''; echo _(' remaining');?><br/>
		    	
		    	<?php if($aws_keys_available=='true'):?>
		    	<strong>SES sends left</strong>: <span id="sends_left"><?php echo $ses_sends_left.' of '.$ses_quota;?></span><br/>
		    	...
		    	<!-- over limit msg -->
		    	<div class="alert alert-error" id="over-limit" style="display:none;">
				  <?php echo _('You can\'t send more than your SES daily limit. Either wait till Amazon replenishes your daily limit in the next 24 hours, or');?> <a href="http://aws.amazon.com/ses/extendedaccessrequest" target="_blank" style="text-decoration:underline">request for extended access</a>. 
				</div>
		    	<!-- /over limit msg -->
		    	...
		    	<div class="alert alert-info">
			    	<p><strong>Note:</strong> We recommend <a href="#cron-instructions" data-toggle="modal" style="text-decoration:underline">setting up CRON</a> to send your newsletters. <?php echo _('Newsletters sent via CRON have the added ability to automatically resume sending when your server times out. You\'ll also be able to schedule emails.');?></p>
			    	...
			    	<p><a href="#cron-instructions" data-toggle="modal" style="text-decoration:underline">Setup CRON now &rarr;</a></p>
		    	</div>
		    	<?php endif;?>
	    ...
	    <div id="cron-instructions" class="modal hide fade">
            <div class="modal-header">
              ...
              <h3><i class="icon icon-time" style="margin-top: 5px;"></i> Add a cron job</h3>
            </div>
            <div class="modal-body">
            <p><?php echo _('To schedule campaigns or to make sending more reliable, add a');?> <a href="http://en.wikipedia.org/wiki/Cron" target="_blank" style="text-decoration:underline">cron job</a> <?php echo _('with the following command.');?></p>
            <h3>Command</h3>
            ...
            <div class="modal-footer">
              <a href="#" class="btn btn-inverse" data-dismiss="modal"><i class="icon icon-ok-sign"></i> Okay</a>
            </div>
          </div>
          <script type="text/javascript">
          	$(document).ready(function() {
	    ...
	    <div class="well" id="schedule-form-wrapper" <?php echo $schedule_form_style;?>>
	    	<?php if(get_app_info('is_sub_user')):?>
			    <?php if(paid()):?>
			    <form action="{{ STATIC_URL }}/includes/create/send-later.php" method="POST" accept-charset="utf-8" id="schedule-form">
			    <input type="hidden" name="total_recipients2" id="total_recipients2">
		    	<?php else:?>
			    <form action="{{ STATIC_URL }}/payment" method="POST" accept-charset="utf-8" id="schedule-form">
			...
			<?php else:?>
				...
				<input type="hidden" name="total_recipients2" id="total_recipients2">
		    <?php endif;?>
		    	<h3><i class="icon-ok icon-time" style="margin-top:5px;"></i> Schedule this campaign</h3><br/>
	    		...
	    		<label for="send_date">Pick a date</label>
	    		<?php 
	    			if($send_date=='')
	    			{
		    			$tomorrow = time()+86400;
				...
				<?php if(get_app_info('is_sub_user')):?>
			        <?php if(paid()):?>
					<button type="submit" class="btn btn-inverse btn-large" id="schedule-btn"><i class="icon-ok icon-time icon-white"></i> Schedule campaign now</button>
					<?php else:?>
					<button type="submit" class="btn btn-inverse btn-large" id="schedule-btn"><i class="icon-arrow-right icon-white"></i> Schedule and pay for campaign</button>
					<?php endif;?>
				<?php else:?>
			    	<button type="submit" class="btn btn-inverse btn-large" id="schedule-btn"><i class="icon-ok icon-time icon-white"></i> Schedule campaign now</button>
				<?php endif;?>
	    ...
	    <div id="edit-newsletter"><a href="<?php echo get_app_info('path')?>/edit?i=<?php echo get_app_info('app')?>&c=<?php echo $cid;?>" title=""><i class="icon-pencil"></i> Edit newsletter</a></div>
    </div>   
    
    <div class="span7">
    	<div>
	    	<h2>Newsletter preview</h2><br/>
	    	<blockquote><strong>From</strong> <span class="label"><?php echo get_saved_data('from_name');?> &lt;<?php echo get_saved_data('from_email');?>&gt;</span></blockquote>
	    	<blockquote><strong>Subject</strong> <span class="label"><?php echo get_saved_data('title');?></span></blockquote>
	    	<blockquote><strong>Opens tracking</strong> <?php echo get_saved_data('opens_tracking') ? '<span class="label label-success">Enabled</span>' : '<span class="label">Disabled</span>';?></blockquote>
	    	<blockquote><strong>Clicks tracking</strong> <?php echo get_saved_data('links_tracking') ? '<span class="label label-success">Enabled</span>' : '<span class="label">Disabled</span>';?></blockquote>
	    	<?php 
			    ...
			    ?>
								<ul id="attachments" style="margin-top: 10px;">
									<li id="attachment<?php echo $i;?>" style="background: white; padding: 0px;">
										...
										<a href="{{ STATIC_URL }}/includes/create/delete-attachment.php" data-filename="<?php echo $file;?>" title="Delete" id="delete<?php echo $i;?>"><i class="icon icon-trash"></i></a>
										<script type="text/javascript">
											$("#delete<?php echo $i?>").click(function(e){
												...
												url = $(this).attr("href");
												c = confirm('Confirm delete \"'+filename+'\"?');
	        ...
	        ?>
	    	<iframe src="{{ STATIC_URL }}/w/<?php echo short($cid);?>?<?php echo time();?>" id="preview-iframe"></iframe>
    	</div>
    </div>
</div>
...
<div class="modal-body">
    <div class="well" style="float:left;">
    	<img src="{{ STATIC_URL }}/img/loader.gif" style="float:left; margin-right:5px; width: 16px;"/> 
...
<div class="modal-body">
    ...
    <p>Or troubleshoot with this FAQ &rarr; <a href="https://sendy.co/troubleshooting#bounces-complaints-warning" target="_blank"><u>https://sendy.co/troubleshooting#bounces-complaints-warning</u></a>.</p></div>
</div>
<div class="modal-footer">
  <a href="#" class="btn btn-inverse" data-dismiss="modal"><?php echo _('Don\'t send');?></a>
  <a href="#" class="btn" data-dismiss="modal" id="send-anyway">Send anyway</a>
</div>
</div>
